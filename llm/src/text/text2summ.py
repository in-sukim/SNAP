from llm.src.utils.text import llmUtils, scores
...
def get_summarization_experiment(df, save_name, lower, upper, df_name, iter_num=5):
    ...
    for i in range(iter_num):
        ...
        df.to_csv(
            os.path.join(OUT_DIR, f"{save_name}_{lower}_{upper}_{df_name}_{i}.csv"),
            index=False,
        )
...
def get_score(save_name, lower, upper, df_name, n):
    df_name = df_name.split("/")[-1]
    model_avg_rouge = scores.get_rouge_list_from_all_df(
        save_name, lower, upper, df_name
    )
    print(model_avg_rouge)
    scores.save_rouge_avg(model_avg_rouge, f"{save_name}_{lower}_{upper}_{df_name}_{n}")
    scores.statistic_from_rouge_list(
        f"{save_name}_{lower}_{upper}_{df_name}_{n}_result.npy"
    )
...
def create_map_reduce_chain(prompt, hf, return_map_results=False):
    map_template = PromptTemplate(
        template=prompts.prompt_extsum_paper2(), input_variables=["document"]
    )
    map_chain = LLMChain(llm=hf, prompt=map_template)

    reduce_template = PromptTemplate(template=prompt, input_variables=["ext_sum"])
...
def summarize_mapreduce(input, prompt):
    MODEL_NAME = "90stcamp/Mistral_7B_Instruct_v0.2_4_bit"
    cache_dir = MODEL_DIR
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_num", "-mn", required=True, type=int)
    parser.add_argument("--data_num", "-dn", required=True, type=int)
    parser.add_argument("--lower", "-l", required=True, type=int)
    parser.add_argument("--upper", "-u", required=True, type=int)
    parser.add_argument("--sample_n", "-sn", required=True, type=int)
    parser.add_argument("--iter_n", default=3, type=int)
    parser.add_argument("--save_name", required=True, type=str)
    parser.add_argument("--max_new_token", default=100, type=int)
    ...
    dataset = llmUtils.load_data(
        config["data_name"][args.data_num], lower=lower, upper=upper
    )
    if len(dataset) < n:
        n = len(dataset)
    tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME, cache_dir=cache_dir)
    ...
    result_df = get_summarization_experiment(
        sample, save_name, lower, upper, config["data_name"][args.data_num], iter
    )
